...
export default function CommunityPopup() {
  ...
  return (
    <div className={`fixed bottom-6 right-6 z-[9999] transition-all duration-500 ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
      <div className="w-72 rounded-2xl shadow-2xl overflow-hidden border border-border glass-card">
        {/* Header */}
        <div className="bg-gradient-to-r from-green-500 to-emerald-500 px-4 py-3 flex items-center justify-between">
          ...
          <button
            onClick={dismiss}
            className="text-white/80 hover:text-white transition-colors rounded-full hover:bg-white/20 p-0.5"
        ...
        <div className="p-4">
          <p className="text-xs text-muted leading-relaxed mb-4">
            Get support, tips, and early features directly on WhatsApp. Join 1,000+ Paylor merchants!
          </p>
          <a
            ...
            onClick={dismiss}
            className="flex items-center justify-center gap-2 w-full bg-green-500 hover:bg-green-600 text-white font-semibold text-xs py-2.5 rounded-xl transition-colors"
          >
            <WhatsAppIcon />
            Join Now
          </a>
          <button
            onClick={dismiss}
            className="mt-2 w-full text-center text-xs text-muted hover:text-main transition-colors py-1"
          >
            Not now
          </button>
        </div>
