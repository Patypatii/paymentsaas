        ...
        <DashboardLayout title="Payment Channels - Paylor">
            <div className="space-y-6">
                <div className="sm:flex sm:items-center sm:justify-between">
                    <div>
                        <h1 className="text-2xl font-bold tracking-tight text-white">Payment Channels</h1>
                        <p className="mt-2 text-sm text-gray-400">Manage your Tills and Paybills for collecting payments.</p>
                    </div>
                    <button
                        onClick={() => setIsAddModalOpen(true)}
                        className="mt-4 sm:mt-0 flex items-center justify-center gap-2 bg-primary hover:bg-primary-hover text-white px-4 py-2 rounded-lg text-sm font-medium transition-all shadow-lg hover:shadow-primary/25"
                    >
                ...
                <div className="flex gap-4">
                    <div className="relative flex-1 max-w-sm">
                        ...
                        <input
                            ...
                            className="w-full bg-[#0B0F1A]/50 border border-white/10 rounded-lg pl-10 pr-4 py-2 text-sm text-white focus:outline-none focus:border-primary/50 transition-all"
                        />
                    </div>
                </div>
                ...
                <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                    {filteredChannels.map((channel) => (
                        <div key={channel.id} className="glass-card p-6 rounded-xl border border-white/10 hover:border-white/20 transition-all group relative overflow-hidden">
                            ...
                            <div className="flex items-start gap-4 mb-4">
                                ...
                                <div>
                                    ...
                                    <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium mt-1 ${channel.status === 'ACTIVE' ? 'bg-green-500/10 text-green-400' : 'bg-gray-500/10 text-gray-400'
                                        }`}>
                                        {channel.status}
                                    </span>
                                </div>
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="flex justify-between text-sm">
                                    <span className="text-gray-500">
                                        {channel.type === 'BANK' ? 'Paybill' : 'Number'}
                                    </span>
                                    <span className="text-white font-mono">{channel.number}</span>
                                </div>
                                {channel.type === 'BANK' && (
                                    <div className="flex justify-between text-sm">
                                        <span className="text-gray-500">Account</span>
                                        <span className="text-white font-mono">{channel.accountNumber}</span>
                                    </div>
                                )}
                            </div>
                        </div>
                    ...
                    <button
                        onClick={() => setIsAddModalOpen(true)}
                        className="glass-card p-6 rounded-xl border border-white/10 border-dashed hover:border-primary/50 hover:bg-white/5 transition-all flex flex-col items-center justify-center text-center gap-4 group h-full min-h-[200px]"
                    >
                        ...
                        <div>
                            <h3 className="font-medium text-white group-hover:text-primary transition-colors">Add New Channel</h3>
            ...
            {isAddModalOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
                    <div className="glass-card w-full max-w-md p-6 rounded-2xl border border-white/10 relative animate-in fade-in zoom-in duration-200">
                        <h2 className="text-xl font-bold text-white mb-6">Add Payment Channel</h2>
                        ...
                        <form onSubmit={handleAddChannel} className="space-y-4">
                            ...
                            <div className="flex gap-3 mt-8">
                                ...
                                <button
                                    type="submit"
                                    className="flex-1 px-4 py-2.5 rounded-lg bg-primary text-white hover:bg-primary-hover transition-colors font-medium shadow-lg hover:shadow-primary/25"
                                >
                                    Add Channel
                                </button>
                            </div>
